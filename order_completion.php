        ...
        // here our table name is chef,waiter,customer
           ...
           // echo "OREDER PLACED SUCCESFULLY";    
            ...
            {   
                ...
                $chef_id = $current_order["chef_id"];
                $customer_id = $current_order["customer_id"];
                $quantity = $current_order["quantity"];
                $t1 = $current_order["ordered_time"];
                $t2 = $current_order["prepared_time"];
                date_default_timezone_set('Asia/Kolkata');
                $delivered_time =date('Y-m-d H:i:s');
                $sql = "INSERT INTO completed_orders  VALUES ('$order_id','$menu_id','$customer_id','$chef_id','$waiter_id','$t1','$t2','$delivered_time')";
                if(mysqli_query($conn, $sql)){
                    echo "OREDER UPADTED SUCCESFULLY";
                    $delete_order = "DELETE FROM waiter_orders where order_id = $order_id";
                    if(mysqli_query($conn, $delete_order))
                    {

                    }
                    echo '<a href="waiter_working_page.php">prepare more</a>';
                }
                else{
                ...
                echo "PLEASE DELIVER A VALID ORDER 1 " 
                    . mysqli_error($conn);
                }
            }
        // Close connection
